...
# tidy input peakfiles to remove peaks in blacklisted regions 
# and non-standard chromosomes 
...
# get appropriate genome annotation
...
if (!is.null(params$picard_files)){
  ...
  # save output 
  if(params$save_output){
    write.table(fragment_info_df, file=paste0(outfile_dir,"/fragment_info"), 
                sep="\t")
  }
}   
```
...
if (params$chrmHMM_plot){
  ...
  if(params$save_output){
    if(params$interact){
      htmlwidgets::saveWidget(plotly::as_widget(samples_chrmHMM), 
                            file = paste0(outfile_dir,"/samples_chrmHMM.html"))
      unlink(paste0(outfile_dir,"/samples_chrmHMM_files"), recursive = TRUE)
    }else{
      ggplot2::ggsave(filename = "samples_chrmHMM.png", plot = samples_chrmHMM, 
                      device = "png", path = outfile_dir)
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
    ...
    for(file in peaklist_tidy){
      sample_in_ref <- IRanges::subsetByOverlaps(x = file, 
    ...
    if(params$save_output){
      if(params$interact){
        htmlwidgets::saveWidget(plotly::as_widget(sample_in_ref_chrmHMM), 
                                file = paste0(outfile_dir,"/sample_in_ref_chrmHMM.html"))
        unlink(paste0(outfile_dir,"/sample_in_ref_chrmHMM_files"), recursive = TRUE)
      }else{
        ggplot2::ggsave(filename = "sample_in_ref_chrmHMM.png", 
                        plot = sample_in_ref_chrmHMM, device = "png", 
                        path = outfile_dir)
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
    ...
    if(params$save_output){
      if(params$interact){
        htmlwidgets::saveWidget(plotly::as_widget(ref_in_sample_chrmHMM), 
                                file = paste0(outfile_dir,"/ref_in_sample_chrmHMM.html"))
        unlink(paste0(outfile_dir,"/ref_in_sample_chrmHMM_files"), recursive = TRUE)
      }else{
        ggplot2::ggsave(filename = "ref_in_sample_chrmHMM.png", 
                        plot = ref_in_sample_chrmHMM, 
                        device = "png", path = outfile_dir)
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
      ...
      # obtain unique peaks 
      for(file in peaklist_tidy){
        sample_not_in_ref <- IRanges::subsetByOverlaps(x = file, 
                                                  ranges = reference_tidy[[1]], 
                                                  invert = TRUE)
      ...
      if(params$save_output){
        if(params$interact){
          htmlwidgets::saveWidget(plotly::as_widget(sample_not_in_ref_chrmHMM), 
                                  file = paste0(outfile_dir,"/sample_not_in_ref_chrmHMM.html"))
          unlink(paste0(outfile_dir,"/sample_not_in_ref_chrmHMM_files"), 
                 recursive = TRUE)

        }else{
          ggplot2::ggsave(filename = "sample_not_in_ref_chrmHMM.png", 
                          plot = sample_not_in_ref_chrmHMM, device = "png", 
                          path = outfile_dir)
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
      ...
      # obtain unique peaks 
      for(file in peaklist_tidy){
        ref_not_in_sample <- IRanges::subsetByOverlaps(x = reference_tidy[[1]], 
                                                       ranges = file, 
                                                       invert = TRUE)
      ...
      if(params$save_output){
        if(params$interact){
          htmlwidgets::saveWidget(plotly::as_widget(ref_not_in_sample_chrmHMM), 
                                  file = paste0(outfile_dir,"/ref_not_in_sample_chrmHMM.html"))
          unlink(paste0(outfile_dir,"/ref_not_in_sample_chrmHMM_files"), 
                 recursive = TRUE)
        }else{
          ggplot2::ggsave(filename = "ref_not_in_sample_chrmHMM.png", 
                          plot = ref_not_in_sample_chrmHMM, 
                          device = "png", path = outfile_dir)
