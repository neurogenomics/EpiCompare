...
# tidy input reference peakfile if provided and include in peaklist
reference_tidy <- params$reference
...
[EpiCompare](https://github.com/neurogenomics/EpiCompare) compares epigenetic datasets for quality control and benchmarking purposes. 
The report consists of three sections: 

1. **General Metrics:** Metrics on peaks (percentage of blacklisted and non-standard peaks, and peak widths) 
...
* Blacklisted peaks removed (%): Percentage of blacklisted peaks in samples. E.g. [ENCODE hg19 blacklist](https://www.encodeproject.org/files/ENCFF000KJP/) 
includes regions in the hg19 genome that have anomalous and/or unstructured signals independent of the cell-line or experiment.
* Non-standard peaks removed (%): Percentage of peaks in non-standard and/or mitochondrial chromosomes.
...
See [here](https://jokergoo.github.io/ComplexHeatmap-reference/book/upset-plot.html) on how to interpret the plot.   
...
if (params$chrmHMM_plot){
  ...
  if(params$save_output){
    if(params$interact){
      htmlwidgets::saveWidget(plotly::as_widget(samples_chrmHMM), file = paste0(outfile_dir,"/samples_chrmHMM.html"))
      unlink(paste0(outfile_dir,"/samples_chrmHMM_files"), recursive = TRUE)
    }else{
      ggplot2::ggsave(filename = "samples_chrmHMM.png", plot = samples_chrmHMM, device = "png", path = outfile_dir)
      unlink("samples_chrmHMM.png")
    }
  }
}else{
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
    ...
    sample_in_ref_chrmHMM <- plot_chrmHMM(peaklist = sample_in_ref_list,
                                          ...
                                          interact = params$interact)
    if(params$interact){
      sample_in_ref_chrmHMM_plot <- htmltools::tagList(plotly::as_widget(sample_in_ref_chrmHMM))
      print(sample_in_ref_chrmHMM_plot)
    }else{
    ...
    if(params$save_output){
      if(params$interact){
        htmlwidgets::saveWidget(plotly::as_widget(sample_in_ref_chrmHMM), file = paste0(outfile_dir,"/sample_in_ref_chrmHMM.html"))
        unlink(paste0(outfile_dir,"/sample_in_ref_chrmHMM_files"), recursive = TRUE)
      }else{
        ggplot2::ggsave(filename = "sample_in_ref_chrmHMM.png", plot = sample_in_ref_chrmHMM, device = "png", path = outfile_dir)
        unlink("sample_in_ref_chrmHMM.png")
      }
    }
  }
}
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
    ...
    ref_in_sample_chrmHMM <- plot_chrmHMM(peaklist = ref_in_sample_list,
                                          ...
                                          interact = params$interact)
    if(params$interact){
      ref_in_sample_chrmHMM_plot <- htmltools::tagList(plotly::as_widget(ref_in_sample_chrmHMM))
      print(ref_in_sample_chrmHMM_plot)
    }else{
    ...
    if(params$save_output){
      if(params$interact){
        ...
        ggplot2::ggsave(filename = "ref_in_sample_chrmHMM.png", plot = ref_in_sample_chrmHMM, device = "png", path = outfile_dir)
        unlink("ref_in_sample_chrmHMM.png")
      }
    }
  }
}
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
      ...
      sample_not_in_ref_chrmHMM <- plot_chrmHMM(peaklist = sample_not_in_ref_list,
                                                ...
                                                interact = params$interact)
      if(params$interact){
        sample_not_in_ref_chrmHMM_plot <- htmltools::tagList(plotly::as_widget(sample_not_in_ref_chrmHMM))
        print(sample_not_in_ref_chrmHMM_plot)
      }else{
      ...
      if(params$save_output){
        if(params$interact){
          htmlwidgets::saveWidget(plotly::as_widget(sample_not_in_ref_chrmHMM), file = paste0(outfile_dir,"/sample_not_in_ref_chrmHMM.html"))
          unlink(paste0(outfile_dir,"/sample_not_in_ref_chrmHMM_files"), recursive = TRUE)

        }else{
          ggplot2::ggsave(filename = "sample_not_in_ref_chrmHMM.png", plot = sample_not_in_ref_chrmHMM, device = "png", path = outfile_dir)
          unlink("sample_not_in_ref_chrmHMM.png")
          }
      }
  }
}
...
if (params$chrmHMM_plot){
  if (is.null(params$reference)==FALSE){
      ...
      ref_not_in_sample_chrmHMM <- plot_chrmHMM(peaklist = ref_not_in_sample_list,
                                                ...
                                                interact = params$interact)
      if(params$interact){
        ref_not_in_sample_chrmHMM_plot <- htmltools::tagList(plotly::as_widget(ref_not_in_sample_chrmHMM))
        print(ref_not_in_sample_chrmHMM_plot)
      }else{
      ...
      if(params$save_output){
        if(params$interact){
          ...
          ggplot2::ggsave(filename = "ref_not_in_sample_chrmHMM.png", plot = ref_not_in_sample_chrmHMM, device = "png", path = outfile_dir)
          unlink("ref_not_in_sample_chrmHMM.png")
        }
      }
  }
}
...
EpiCompare uses `ChIPseeker::annotatePeak()` to annotate peaks with the nearest gene and genomic regions where 
the peak is located. 
...
`ChIPseeker::annotatePeak()` is first used to assign peaks to nearest genes.
Biological themes amongst the genes are identified using ontologies (KEGG and GO). 
The peaks are annotated with [genes](http://bioconductor.org/packages/release/data/annotation/html/TxDb.Hsapiens.UCSC.hg19.knownGene.html) taken from the annotations of human genome hg19 provided by Bioconductor.
...
The frequency of peaks in downstream (-3000bp) and upstream (+3000bp) of TSS is plotted. 
Faint color line around the main frequency line represents the the 95% confidence interval estimated by bootstrap method. 
...
if (params$tss_plot){
  ...
  # save output 
  if(params$save_output){
    ggplot2::ggsave(filename = "TSS.png", plot = tss_plot, device = "png", path = outfile_dir)
    unlink("TSS.png")
  }
}
```
