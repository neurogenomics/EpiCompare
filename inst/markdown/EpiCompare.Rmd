...
```{r,fig.width=7,warning=FALSE,message=FALSE,fig.height=fig_height, results='asis'}
if(params$precision_recall_plot && (!is.null(reference))){
    pr_out <- plot_precision_recall(peakfiles = peaklist_tidy, 
                                    ...
                                    interact = params$interact, 
                                    show_plot = FALSE)
    methods::show(pr_out$precision_recall_plot)
    methods::show(pr_out$f1_plot)
}
```
