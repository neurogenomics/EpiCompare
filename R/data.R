...
#' \code{encode_H3K27ac <- encode_H3K27ac[seqnames(encode_H3K27ac) == "chr1"]} \cr
#' \code{my_label <- c("name","score","strand","signalValue","pValue","qValue","peak")} \cr
...
#' The code to prepare the .Rda file from the raw peak file is:
#'
#' \code{# sequences were directly downloaded from https://trace.ncbi.nlm.nih.gov/Traces/sra/?run=SRR8383507} \cr
#' \code{# and peaks (BED file) were generated by Leyla Abbasova (Neurogenomics Lab, Imperial College London)} \cr
...
#' The code to prepare the .Rda file from the raw peak file is:
#'
#' \code{# sequences were directly downloaded from https://trace.ncbi.nlm.nih.gov/Traces/sra/?run=SRR8581604} \cr
#' \code{# and peaks (BED file) were generated by Leyla Abbasova (Neurogenomics Lab, Imperial College London)} \cr
...
#'
#' \code{picard <- read.table("path/to/picard/duplication/output", header = TRUE, fill = TRUE)]} \cr
...
#' \code{hg19_blacklist <- ChIPseeker::readPeakFile(file.path(path), as = "GRanges")} \cr
...
#' \code{hg38_blacklist <- ChIPseeker::readPeakFile(file.path(path), as = "GRanges")} \cr
